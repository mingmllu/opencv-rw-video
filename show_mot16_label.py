...
def overlay_image_with_detection_result(raw_image, list_det, list_gt):
...
input_file = open(os.path.join(path, 'gt/gt.txt'), 'r')
list_gt = []
for line in input_file:
  fn,id,x,y,w,h,_,_,_ = line.rstrip('\n').split(',')
  fn = int(fn)
  x,y,w,h = float(x),float(y),float(w),float(h)
  if fn == frame_number:
    list_gt.append((id, [int(x),int(y),int(w),int(h)]))
